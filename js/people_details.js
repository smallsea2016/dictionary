    ...
    .done(function(data) {
        ...
        for (var i = 0; i < len; i++) {
            str += '<li class="flexbox">'
                ...
                +        '<input type="checkbox" class="checkbox" data-index="index'+i+'" data-value="'+data.result[i].name+'" value="'+data.result[i].enName+'" hidden><i></i><b class="ellipsis">'+data.result[i].name+'</b>'
                +    '</div>'
                +    '<a href="javascript:;" class="wx_ico"></a>'
                +    '<a href="tel:13800138000" class="tel_ico"></a>'
                + '</li>';
        };
...
function peopleList(obj){
    ...
    .done(function(data) {
        var str = '',
            ...
            mt = +new Date(),
            name = $(obj).val(),
            enName = $(obj).attr('data-value');   //用来生成一个唯一的class，便于操作
            str += '<li class="js_'+mt+'">'
                +'<img src="images/user2.jpeg">'
                +'<div>'
                +    '<b class="ellipsis">'+name+'</b>'
                +    '<b class="ellipsis">'+enName+'</b>'
                +'</div>'
